...
Task 6 WorkManager Slice 1: AndroidX WorkManager scheduling added with deterministic `kairo-ingestion-<sessionId>` unique work, `ExistingWorkPolicy.KEEP`, session-ID-only input, custom `KairoWorkerFactory`, and Android-owned `IngestionRuntimeFactory` seam. Focused `IngestionWorkSchedulerTest.enqueues one unique work item for a session` passed.
