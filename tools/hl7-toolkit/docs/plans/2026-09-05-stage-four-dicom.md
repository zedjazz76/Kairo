...
2. **DICOM workspace:** add local-file metadata inspection, search, summary/findings, and redacted copy UI using existing patterns. **User-test checkpoint ready:** local Part 10 file selection, metadata table, tag search, SOP explanation, summary, findings, and reviewed redacted copy are integrated in the existing DICOM navigation workspace. Redacted copy retains only recognized SOP class/Transfer Syntax/modality constants; other metadata is omitted. Automated file-handler/copy checks pass; native browser chooser/clipboard acceptance remains pending.
